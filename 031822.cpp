...
////////////////////////////////////////////// ROTARY INPUTS
  ...
  if(digitalRead(TOGGLE_SWITCH2)==LOW)// Function switch to F13-F17
    ...
    if(digitalRead(KEY_PIN1)==LOW)
      {
      Keyboard.write(HID_KEYBOARD_ESCAPE); // Function Key F13
        delay(50);
      }
      
    if(digitalRead(KEY_PIN2)==LOW)
      {
      Keyboard.write(HID_KEYBOARD_LEFT_CONTROL); // Function Key F14
        delay(150);
      }
      
    if(digitalRead(KEY_PIN3)==LOW) 
      {
      Keyboard.write(KEY_LEFT_SHIFT); // Function Key F15
        delay(150);
      }

    if(digitalRead(KEY_PIN4)==LOW)
      {
      Keyboard.write(KEY_LEFT_ALT); // Function Key F16
        delay(150);
      }
    while(digitalRead(KEY_PIN5)==LOW)
      {
      Keyboard.write(KEY_SCROLL_LOCK); // Function Key F17
      }
